...
```sh
   git clone https://github.com/your_username/deep_q_learning_breakout.git
   cd deep_q_learning_breakout
```

2. Create and activate a virtual environment:
```sh
    python3.9 -m venv myenv_breakout

    source myenv_breakout/bin/activate  # For macOS/Linux

    myenv_breakout\Scripts\activate     # For Windows
```
...
- To play Atari Breakout using a pre-trained model, run the play.py script:

```sh
python play.py
```
You can modify the parameters in the play.py script to change the environment name, model weights file, number of episodes to play, and render mode.

### play.py Parameters

- `env_name` (str): Name of the gym environment to use (default: 'Breakout-v4').
- `model_weights` (str): Path to the pre-trained model weights (default: 'policy.h5').
- `episodes` (int): Number of episodes to play (default: 10).
- `render_mode` (str): Mode to render the environment (default: 'human').

## Project Structure

- `train.py`: Main script for training the agent.
- `play.py`: Script to play the game using a pre-trained model.
- `README.md`: Project documentation.
- `requirements.txt`: List of Python dependencies.

## Model Details

The model uses a convolutional neural network to process input images and a DQN agent to learn how to play Breakout. The hyperparameters and model architecture are defined in train.py.

## Notes

- The train.py script uses gym for the game environment and keras-rl2 for the DQN implementation.
- Ensure that the versions of the libraries match those specified in requirements.txt to avoid compatibility issues.

## Troubleshooting

### Error: `ImportError: cannot import name '__version__' from 'tensorflow.keras'`
...
# Rest of the file...
```
Below is an example of the completely modified callbacks.py file:
...
Deprecation warnings may appear due to changes in the APIs of the used libraries. These warnings can generally be ignored, but it is recommended to address them to ensure future compatibility.

## Author

Caroline CHOCHOY

Connect with me on [LinkedIn](https://www.linkedin.com/in/caroline-chochoy62/)
